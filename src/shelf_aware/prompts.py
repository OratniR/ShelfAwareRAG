...
2. 期間に幅がある場合や、複数の情報源がある場合は、言及されているすべての日数を extracted_days に含めてください。
...
5. extracted_days は必ず日単位の数値のみにしてください（1年=365, 半年=182, 1ヶ月=30, 1週間=7）。
6. 検索結果に具体的な期間が書かれていない場合は extracted_days を [] にしてください。推測で数値を作ってはいけません。
7. is_food が true のときも extracted_days と reason のキーは必ず含めてください。

# 出力形式（厳守）
- JSONオブジェクトを1つだけ出力する。前後に説明文・挨拶・Markdownのコードフェンス(```)を書かない。
- reason は日本語で40文字以内。
- 下の出力例の数値をそのまま使わないこと。必ずContextの記述に基づくこと。
...
- extracted_days: 未開封での保存期間を日数に換算した数値の配列（1年=365, 半年=182, 1ヶ月=30, 1週間=7）。記載がなければ []
- reason: 日本語40文字以内
