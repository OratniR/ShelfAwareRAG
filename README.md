...
補足: イメージに `tzdata` は入れていない（入れると apt レイヤが変わり `uv sync` の
レイヤキャッシュが無効化されて再ビルドが長時間化するため）。JSTは
...
コード側 (`estimation.py`) で行う。小型モデルは「2年」を `2` と出力しがちで、それを日数として
扱うと **2日** になり誤った「期限切れ」警告が出るため。

- 単位を落とした数値は、同じ数値が「n年」「nヶ月」「n週間」として応答中に現れていれば
  その単位で解釈し直す（例: `extracted_days=[0, 2]` + reason「未開封で2年」→ 730日）
- 単位の根拠が無い小さい数値は、表記そのままを求めて1回だけ聞き直す
- それでも単位が確定しない場合は日数として扱い、警告ログとLangfuseの `unitless_days` に残す

### 食品判定（「食べ物なのに対象外になる」を防ぐ）

1. 語尾が `豆腐` `塩` `酢` `料理酒` `ソース` など既知の食べ物 → **LLMに聞かずに food**（辞書）
2. LLMには **JSON**（`{"is_food": true}`）で答えさせる
   （以前は「non-food という語が含まれるか」で判定していたため、小型モデルが
    「『food』または『non-food』の一単語で」という制約文を復唱しただけで誤判定していた）
3. 非食品と判定されたら **別の聞き方（はい/いいえ）で確認**し、2回とも非食品のときだけ
   「対象外」にする。判定が割れた場合は食べ物として扱う（見逃しより誤除外を避ける）

誤って「🚫 対象外」になったアイテムは、ダッシュボードでステータスを「🕒 未処理」に戻すと
再推定の対象になります。
...
| 表示 | カラム | 意味 |
| --- | --- | --- |
| 登録日 | `created_at` | 最初に登録した日時（再登録しても変わらない） |
| 更新日 | `updated_at` | 最後に内容が変わった日時 |
| 賞味期限 | `expiry_date` | 「登録（再登録）した日 + 推定した日数」 |

同じ名前で再登録すると**新しく買った在庫**として扱い、賞味期限を推定し直す（古い期限を
残すと「登録日より前に賞味期限がくる」状態になるため）。単に場所を移しただけの場合は、
ダッシュボードで賞味期限を直接編集してください。

なお、ダッシュボードの保存は全行に対して更新をかけるため、**値が変わっていない行の
`updated_at` は書き換えない**ようにしてある（以前は保存のたびに全行の登録日が動いていた）。
...
| `TZ` | `JST-9` | コンテナのタイムゾーン。POSIX形式なのでイメージに tzdata を入れる必要がない（`Asia/Tokyo` でも動くが、その場合は tzdata が必要） |
